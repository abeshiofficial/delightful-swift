...
export const DailyStackedChart = ({
  ...
  const getSegmentHeight = (minutes: number) => {
    ...
    <div className="mt-6">
      ...
      <div className="flex items-end justify-between gap-2" style={{ height: chartHeight }}>
        {processedData.map((day, dayIndex) => {
          const totalHeight = day.segments.reduce(
            (sum, seg) => sum + getSegmentHeight(seg.minutes),
            0
          );
              ...
              >
                {day.segments.map((segment, segIndex) => (
                  <motion.div
                    ...
                    className="w-full"
                    style={{
                      height: getSegmentHeight(segment.minutes),
